...
class CheckoutPDV extends TWindow
{
    ...
    function __construct()
    {
        ...
        if(parent::isMobile())
        {
            ...
            $obs = new TText('obs');
            $obs->placeholder= 'Faça as obsservações aqui EX. Não colocar cebola, bem passado...';
            ...
            $obs->setSize( '100%', 100);
            $obs->setOption('placeholder', 'Faça as obsservações aqui EX. Não colocar cebola, bem passado...');
            ...
            if ($dados->type == 1)
            {
                ...
                if($cart_items)
                {
                    ...
                    foreach ($cart_items as $id => $amount)
                    {
                        ...
                        $itemVenda->sale_price = $product->sale_price;
                        $itemVenda->total = $amount * $product->sale_price;
                ...
                $venda_aberta = Sale::where('mesa', '=', $dados->mesa)->where('status', '=', '1')->load();
                if(!empty($dados->mesa) and empty($venda_aberta) ){
                  ...
                  //  $venda->datepag = date('d-m-Y');
                    ...
                    if($cart_items)
                    {
                        ...
                        foreach ($cart_items as $id => $amount)
                        {
                            ...
                            $itemVenda->product_id =  $product->id;
                            $itemVenda->sale_id =  $venda_aberta[0]->id;
                            $itemVenda->mesa =  $dados->mesa;
                            $venda_aberta[0]->total +=   $itemVenda->total;
                            $itemVenda->store();
                            $venda_aberta[0]->store();
                    ...
                    new TMessage('info', 'Itens adionado a mesa com sucesso!');
